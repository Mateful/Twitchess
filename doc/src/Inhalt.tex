...
\chapter{Einführung}
	...
	\section{Akronym Twitchess}
		...
		Unsere Aufgabe war daher es einen \cite{Github}Twitterbot zu entwickeln,
	...
	\section{Anforderungen}
		...
		Features nach, die bei entsprechende Zeit zu integrieren wären. Folgende Punkte
		\begin{itemize}
...
\chapter{Kommandos und Limitierung}
	\section{Befehlskommandos}
		...
		immer als @Name bezeichnet}
			\begin{itemize}
				\item ``@Name chess move e2e4'' oder
				\item ``@Name chess move player e2e4'' 
			\end{itemize}
			\item Spieler lässt seinen Zug von Twitchess machen
			\begin{itemize}
	...
	\section{Limitierungen}
		...
		welches theoretisch ziemlich schnell erschöpft ist. Auf Grund dessen ist es
		auch nicht möglich nach jedem Zug ein neues Bild zu generieren. Da wir aber beim Hochladen
		der Bild unabhängig von anderen Servern sein wollten, wählten wir das Image
		Feature von Twitter. Dies stellt jedoch die einzige Limitierung dar.
\chapter{Spielgeschehen}
	\section{Spielstart}
		...
		Um Twitchess mit dem eigenen Twitteraccount zu verbinden, gib am Anfang
		einfach deinen Benutzernamen ein. Danach erhälst du die Nachricht, dass noch kein Token zu
		deinem Account existiert. Wähle ``neues Token erstellen''. Kopiere den
		erscheinenden Link in deinen Browser und gib somit Twitchess Zugriff auf
		deinen Account. Gib den auf der nächsten Seite erscheinenden Pin in die Konsole ein.
		Jetzt bist du mit deinem Twitter Account eingeloggt. Ab dem nächsten Start
		reicht es deinen Twitternamen einzugeben und du wirst verbunden. 
			\subsubsection{Twitchess entfernen}
			Um deinen Account wieder für Twitchess zu sperren gehe in deinem
			Twitteraccount auf ``Einstellungen'' $\to$ ``Applikationen'' $\to$ ``Name
			des Twitteraccounts'' $\to$ ``Zugriff widerrufen''.
			
			Hinweis: Den Standard-Twitter-Account kannst du in der ``configuration.properties''
			Datei ändern. (``Twitter.StandardAccount=Name'')
	\section{Spielablauf}
		...
		\begin{itemize}
			\item Befehl: ``@Name chess new''
		\end{itemize}
		Wenn alles geklappt hat, erhälst du eine Bestätigung.
	...
	\subsection{Unterstützte Plattformen}
		...
		\begin{itemize}
			...
			\item Ubuntu 10.10
		\end{itemize}
		Dabei wurde	vor allem Aspekt auf die möglichst einfache Ausführbarkeit, sowie Spielablauf
		und -verlauf gelegt. Abstürze gab es bei unseren Tests keine.
...
\chapter{Verwendete Software}
	\section{Verwendete Engine}
		\subsection{Stockfish}
			...
			\cite{StockWeb}Stockfish brauch sich auch gar nicht hinter kommerziellen
		...
		\subsubsection{Lizenz}
			...
			in Grund zur Wahl dieser Engine.
		\subsubsection{Tests}
		...
		\subsubsection{Cross Platforms} 
			...
			\cite{StockWeb}Stockfish von Hause mitliefert. So ziemlich jede große
			Plattform wird unterstützt. Darüber  hinaus gibt es sogar eine
			\cite{SfcApp}App für iPhone, iPod touch, and iPad welche natürlich auf der Stockfishengine beruht.
